...
import com.nabenik.dao.CategoriaFacade;
...
public class CategoriaEndpoint {
    ...
    public List<Categoria> listAll() {
        ...
        final List<Categoria> results = categoriaService.findAll();
        return results;
    }

    @PUT
    @Path("/{id:[0-9][0-9]*}")
    public Response update(@PathParam("id") Integer id, Categoria entity) {
        if (entity == null) {
        ...
        if (categoriaService.find(id) == null) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            entity = categoriaService.edit(entity);
